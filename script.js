...
// page navigation variables (data-page approach)
const navigationLinks = document.querySelectorAll("[data-nav-link]");
const pages = document.querySelectorAll("[data-page]");

// helper: activa la página y el enlace correspondiente
function activatePage(targetPage) {
  pages.forEach(page => page.classList.toggle("active", page.dataset.page === targetPage));
  navigationLinks.forEach(link => link.classList.toggle("active", link.dataset.page === targetPage));
  window.scrollTo(0, 0);
}
...
(function initNavigation() {
  ...
  if (initialPage) {
    const initPageName = initialPage.dataset.page;
    const initLink = document.querySelector(`[data-nav-link][data-page="${initPageName}"]`);
...
// add click event to all nav links (uses data-page)
navigationLinks.forEach(link => {
  link.addEventListener("click", function (e) {
    e.preventDefault();
    const targetPage = this.dataset.page;
    if (!targetPage) return; // protección por si falta data-page
